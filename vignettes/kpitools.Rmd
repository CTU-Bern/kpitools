...
By default, the number of breakpoints is used to create variations on high to low via the `risklabs` function. For instance, giving three to six values as breakpoints produces the following sets of labels. For more than 5 groups (six breakpoints), labels must be defined manually.
...
It's simple to override these labels with the `risklabels` argument (be careful combining `risklabels` and `direction` arguments as the label order will be inverted). 
